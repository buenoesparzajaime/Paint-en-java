...
public class MiRectangulo extends Figura2D{
    public MiRectangulo(){
	}

	public MiRectangulo(int x, int y, boolean estaRelleno, 
                            Paint pintar, Stroke trazo){
            super(x, y, estaRelleno, pintar, trazo);
	}

	public MiRectangulo(int x1, int y1, int x2, int y2, 
                            boolean estaRelleno, Paint pintar, Stroke trazo){
            super(min (x1, x2), min (y1, y2), max (x1, x2), max (y1, y2), 
                  estaRelleno, pintar, trazo);
	}
	
	public void dibujar(Graphics2D grafico){
	    super.dibujar(grafico);
	    Rectangle2D rect = new Rectangle2D.Double (pInicial.x,pInicial.y,
